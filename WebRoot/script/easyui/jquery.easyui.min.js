...
//@update add 信息提示框在下拉列表中往右移19像素
if($(_1e3).parent().hasClass("combo")) {
	pos.left += 19;
}
//end
...
},prompt:function(_29c,msg,fn,type){
if(type=="") type="text";
var _29d="<div class=\"messager-icon messager-question\"></div><div>"+msg+"</div><br/><div style=\"clear:both;\"/><div><input class=\"messager-input\" type=\""+type+"\"/></div>";
//end
var _29e={};
	...
	$.each(opts.queryParams, function(k, v) {
		switch(v.type) {
			case 'text':
				...
				break;
			case 'combo':
				_6ae[k] = $("#" + v.id).combo('getValue');
				break;
			case 'combobox':
				_6ae[k] = $("#" + v.id).combobox('getValue');
				break;
			case 'datebox':
				_6ae[k] =  $("#" + v.id).datebox('getValue');
				break;
			case 'datetimebox':
				_6ae[k] = $("#" + v.id).datetimebox('getValue');
				break;
			case 'numberbox':
				_6ae[k] =  $("#" + v.id).numberbox('getValue');
				break;
			case 'combotree':
				_6ae[k] = $("#" + v.id).combotree('getValue');
				break;
			case 'combogrid':
				_6ae[k] = $("#" + v.id).combogrid('getValue');
				break;
			default:
				_6ae[k] = v;
		}
...
if (opts.pagination) {
	if (opts.paramsName == null) {
		opts.paramsName = {
			page : 'page',
			num : 'num'
		};
	}
	_6ae[opts.paramsName.page] = opts.pageNumber;
	_6ae[opts.paramsName.num] = opts.pageSize;
}
//end
if(opts.sortName){
...
//@update 替换整个loader方法，未登录等出错处理
},loader:function(data, succfn, errfn) {
	var opts = $(this).datagrid("options");
	if (!opts.url) {
		return false;
	}
...
if(_875.children.length==0 && _875.isParent!=true){
//end
