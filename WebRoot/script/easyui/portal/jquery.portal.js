...
(function($){
	...
	function init(target, options){
		...
		$(target).find('>div').each(function(key){	// each column panel
			var column = $(this);
			if(columns[key] == 0) {
				totalWidth += column.outerWidth();
				columnWidths.push(column.outerWidth());
			} else {
				columnWidths.push(columns[key]);
			}
			
			var td = $('<td class="portal-column-td" valign="top"></td>').appendTo(tr);
	...
	function setSize(target){
		...
		var width = t.width();
		var ptl = $.data(target, 'portal');
		var param = {};
		param.width = width;
		param.minwidth = ptl.options.minwidth;
		param.columns = ptl.options.columns;
		param.auclnum = param.columns.length-1;
		$.each(param.columns, function(k, v) {
			if(v != 0) param.width -= v;
			else param.auclnum = k;
		});
		var columnWidths = ptl.columnWidths;
		var leftWidth = 0;
		...
		for(var i=0; i<columnWidths.length; i++){
			...
			if(w <= 1) {
				w = Math.floor(param.width * w);
				if(w < param.minwidth) w = param.minwidth;
			}
			if (i == param.auclnum){
				if(param.columns[i] <= 1) {
					//w -=  (hasScroll == true ? 18 : 0);
					w -= 18;
					if(w < param.minwidth) w = param.minwidth;
				} else
					//w = width - leftWidth - (hasScroll == true ? 18 : 0);
					w = width - leftWidth - 18;
			}
			p._outerWidth(w);
				...
				if (pal[0] != source){
					...
					if (e.pageX > pos.left && e.pageX < pos.left + pal.outerWidth()
						...
						if (e.pageY > pos.top + pal.outerHeight() / 2){
							result = {target:pal, pos:'down'};
						} else {
							result = {target:pal, pos:'up'};
						}
					}
				}
			});
