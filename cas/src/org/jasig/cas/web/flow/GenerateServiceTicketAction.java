...
public final class GenerateServiceTicketAction extends AbstractAction {
    ...
    protected Event doExecute(final RequestContext context) {
        ...
        try {
            ...
            //如果是检查登录状态，那么返回json数据 @update
            if(context.getRequestParameters().get("j_ajax_check") != null) {
        		String userName = this.centralAuthenticationService.getUserName(serviceTicketId);
        		System.out.println("Join to hasLogin! userName=" + userName);
        		context.getRequestScope().put("userName", userName);
        		return result("loginCheck");
        	}
            return success();
        ...
        /*try {
        	...
        	credentials.setUsername("b051");
        	credentials.setPassword("noPass");
        	//生成TGT，服务端保存一份
