    ...
    public final String submit(final RequestContext context, final Credentials credentials, final MessageContext messageContext) throws Exception {
        ...
        if(context.getRequestParameters().get("j_ajax") != null) {
        	if(! StringUtils.hasText(user.getUsername())) {
        		context.getRequestScope().put("loginMess", "noAccount");
        		//System.out.println("Join to loginError! mess=User name is empty!");
        		return "loginError";
        	}
        	if(! StringUtils.hasText(user.getPassword())) {
        		context.getRequestScope().put("loginMess", "noPass");
        		//System.out.println("Join to loginError! mess=Password is empty!");
        		return "loginError";
        	}
        }
        //把用户名转换成小写  @update
        user.setUsername(
        		StaticMethod.tomcatDecode(WebUtils.getHttpServletRequest(context), user.getUsername())
        	.toLowerCase());
    	//对rsa解密 @update
    	//System.out.println("username: " + user.getUsername());
    	//System.out.println("password: " + user.getPassword());
    	try {
    		RSAPrivateKey privateKey = (RSAPrivateKey) context.getFlowScope().remove("rsa_priKey");
    		//System.out.println("privateKey: " + privateKey.getPrivateExponent().toString(16));
    		String decode = RSACoder.decryptByPrivateKey(user.getPassword(), privateKey);
    		//System.out.println("解密: " + decode);
    		decode = StaticMethod.makePass(user.getUsername(), decode);
    		//System.out.println("加密: " + decode);
    		user.setPassword(decode);
    	} catch(Exception e) {
    		this.logger.warn("Rsa can not decode!");
    		//如果参数包含j_ajax，那么返回json数据 @update
            if(context.getRequestParameters().get("j_ajax") != null) {
            	context.getRequestScope().put("loginMess", "rsaError");
            	//System.out.println("Join to loginError! mess=rsa decode error");
            	return "loginError";
            }
    		return "error";
    	}
        ...
        if (!authoritativeLoginTicket.equals(providedLoginTicket)) {
            ...
            //如果参数包含j_ajax，那么返回json数据 @update
            if(context.getRequestParameters().get("j_ajax") != null) {
            	context.getRequestScope().put("loginMess", "ticketError");
            	//System.out.println("Join to loginError! mess=login ticket error");
            	return "loginError";
            }
            return "error";
        }
        ...
        if (StringUtils.hasText(context.getRequestParameters().get("renew")) && ticketGrantingTicketId != null && service != null) {
        	...
        	/*try {
                ...
                if (isCauseAuthenticationException(e)) {
                    ...
                    //如果参数包含j_ajax，那么返回json数据 @update
                    if(context.getRequestParameters().get("j_ajax") != null) {
                    	context.getRequestScope().put("loginMess", "passError");
                    	//System.out.println("Join to loginError1!");
                    	return "loginError";
                    }
        ...
        try {
            ...
            if(context.getRequestParameters().get("j_ajax") != null) {
            	context.getRequestScope().put("loginMess", "passError");
            	//System.out.println("Join to loginError2!");
            	return "loginError";
            }
            if (isCauseAuthenticationException(e))
                return getAuthenticationExceptionEventId(e);
