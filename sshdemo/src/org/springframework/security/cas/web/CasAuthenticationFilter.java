...
public class CasAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public Authentication attemptAuthentication(final HttpServletRequest request, final HttpServletResponse response)
        ...
        if(param != null) {//进行注册，信息不符合就抛异常
        	if(StaticMethod.checkStr(param, 2, 20)) {
        		User user = new User();
        		user.setName(param);
        		param = request.getParameter("depm");
        		int id = StaticMethod.Str2Int(param);
        		if(id != -1) {
        			user.setDpm(new Department(id));
        ...
        throw new UsernameNotFoundException("User registration failure!");
    }
