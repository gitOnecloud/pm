        ...
        if (session != null) {
            ...
            if (info != null) {
                if (info.isExpired()) {
                    ...
                    if(!targetUrl.endsWith("_rd.action")) {
                    	request.getRequestDispatcher("/json.jsp?json={\"status\":1,\"mess\":\"本账号同时登录数超出限制！\",\"login\":false}")
            				.forward(request, response);
                    	return;
                    }
                    
                    targetUrl = determineExpiredUrl(request, info);

                    if (targetUrl != null) {
                    	//redirect跳转方式会覆盖原来的访问地址
