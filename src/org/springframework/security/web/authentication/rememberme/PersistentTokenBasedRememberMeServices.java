 ...
 * it will not delete these tokens from storage. A suitable batch process should
 * be run periodically to remove expired tokens from the database.
 * </p>
 * 
 * @author Luke Taylor
 * @since 2.0
 */
public class PersistentTokenBasedRememberMeServices extends
	...
	protected UserDetails processAutoLoginCookie(String[] cookieTokens,
		...
		if (!makeTokenSignature(presentedToken, request, token.getUsername(), userDetails.getPassword())
				.equals(token.getTokenValue())) {
			// Token doesn't match series value. Delete all logins for this user
			// and throw an exception to warn them.
			tokenRepository.removeUserTokens(token.getUsername());
			throw new InvalidCookieException("Invalid remember-me token (Series/token) mismatch. Implies previous cookie theft attack.");
		...
		try {
			tokenRepository.updateToken(newToken.getSeries(),
					makeTokenSignature(newToken.getTokenValue(), request,
							token.getUsername(), userDetails.getPassword()),
					newToken.getDate());
	 ...
	 * to the response.
	 * 
	 * @update cookie仍然按照之前保存series和token，
	 *         但数据库保存series和由name、password、ip、browser
	 *         、token生成的MD5，这是为了防止session被盗
	 */
	protected void onLoginSuccess(HttpServletRequest request,
		...
		try {
			tokenRepository.createNewToken(new PersistentRememberMeToken(username, series,
					makeTokenSignature(token, request, username, password), new Date()));
			addCookie(new PersistentRememberMeToken(null, series, token, null),
					request, response);
		} catch (DataAccessException e) {
...
// copy from TokenBasedRememberMeServices.java
	/**
	 * 对session进行标记
	 * @param request 用于获取ip和browser
	 */
	protected String makeTokenSignature(String token,
			HttpServletRequest request, String username, String password) {
		// 加入ip和浏览器信息，对session防止被盗有一定作用
		return DigestUtils.md5DigestAsHex((token + username + password +
				StaticMethod.getRemoteAddr(request) +
				StaticMethod.getBrowser(request)).getBytes());
	}
